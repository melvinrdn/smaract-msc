...
class SmarActMCS:
    ...
    def open(self):
        """
        Automatically discovers and opens the first available MCS system. Fine because we only have one.
        """
    ...
    def step_move(self, channel: int, steps: int, amplitude: int = 4095, frequency: int = 2000):
        ...
        channel : int
            Channel index
        steps : int
            Number of steps. Each step is a signal pulse.
        amplitude : int
            Step amplitude. The lower the number, the smaller the step.
        frequency : int
            Step frequency. The higher the number, the faster the step.
        """
    ...
    def _wait_until_done(self, channel: int):
        """
        Waits until a move or command finishes on a given channel.
        """
        status = c_uint()
        while True:
